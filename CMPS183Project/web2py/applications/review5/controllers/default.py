...
def index():
    ...
    if auth.user is not None:
        privateMemo = db(db.checklist.is_public == False).select()
        publicMemo = db(db.checklist.user_email == auth.user.email).select()
        checklists = privateMemo | publicMemo
    else:
        q = db.checklist.is_public == True
        checklists = db(q).select()
    # print "****LIST*****"
    # print checklists
...
def press():
    if request.args(0) is None:
        ...
        cl = db(q).select()
        # print "******pr4ssw*********"
        # print cl[0].is_public
        #print db(q).select(row[0])
        db(q).update(is_public=not(cl[0].is_public))
    redirect(URL('default', 'index'))
