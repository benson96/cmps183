...
def index():
    ...
    checklists = None #checklists is initially empty
    all_authors = db(db.checklist).select() #Get all lists 
    if auth.user is not None: #if user is logged in, do this:
        checklists = db(db.checklist.user_email == auth.user.email).select() # get all items associated with login email
        all_authors = db(db.checklist and db.checklist.user_email != auth.user.email).select() # get all public items not associated w/ login email
    return dict(checklists=checklists , all_authors = all_authors ) #Put the keys in a dictionary
