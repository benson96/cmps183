...
def index():
    ...
    checklists = None
    users = None
    #This says give me a checklist if there is a logged in user 
    if auth.user is not None:
        checklists = db(db.checklist.user_email == auth.user.email).select()
    else: 
    	users = db(db.checklist.is_public == True).select()
        
    return dict(checklists=checklists, users=users)
...
def toggle_private(): 
	if request.args(0) is not None: 
	#checklist is the table you created
	#.user_email is field in the checklist entry
		q = ((db.checklist.user_email == auth.user.email) &
    	(db.checklist.id == request.args(0)))
...
def toggle_public(): 
	...
	#.user_email is field in the checklist entry
    	...
    	(db.checklist.id == request.args(0)))
    	row = db(q).select().first()
    	row.update_record(is_public='is_public')
